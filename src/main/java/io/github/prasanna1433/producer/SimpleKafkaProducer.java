...
public class SimpleKafkaProducer {
	public static void main(String[] args) {
		String topic = "ghp-topic";

		// defines the properties for the kafka producer
		Properties producerProperties = new Properties();
		// specify the kafka cluster that producer has to connect
		producerProperties.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, KafkaConstant.BOOTSTRAP_SERVERS);
		...
		for (int i = 0; i < 100; i++) {
			// key 프로퍼티를 명시적으로 주면 파티션 key 에 해당하는 파티션에 들어 감
			// producer.send(new ProducerRecord<>(topic, Integer.toString(i % 100), new StringBuilder().append("message number").append(i).toString()));
			producer.send(new ProducerRecord<>(topic, new StringBuilder().append("hello num=").append(i).toString()));
		}
